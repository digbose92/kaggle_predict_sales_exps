...
def generate_shop_data(shops_data):
    ...
    shops_data['shop_category']=le_shops.fit_transform(shops_data['category'])
    shops_data = shops_data[['shop_id','city_id', 'shop_category']]
...
def generate_train_test_pairs(train_data,test_data,items_data,shops_data,category_data):
    ...
    for block_num in range(34):
        ...
        train_data_curr=train_data[train_data.date_block_num==block_num]
        matrix.append(np.array(list(product([block_num], train_data_curr.shop_id.unique(), train_data_curr.item_id.unique())), dtype='int16')) #product here gives the cartesian product of shop id and item id for that month

    stack_data = pd.DataFrame(np.vstack(matrix), columns = cols)
    stack_data.sort_values(cols,inplace=True)

    group_data=train_data.groupby(cols).agg({'item_cnt_day':['sum']}) #aggregates for each month for each (shop,item) pair the number of items sold
    group_data.columns=['target']
    group_data.reset_index(inplace=True)
    tot_data = pd.merge(stack_data, group_data, how='left', on=cols)
    tot_data['target']=(tot_data['target'].fillna(0).clip(0,20).astype(np.float16)) #clipping the target value to (0,20) (same range as )
    ...
    tot_data['city_id'] = tot_data['city_id'].astype(np.int8)
    tot_data['shop_category'] = tot_data['shop_category'].astype(np.int8)
    tot_data['item_category_id'] = tot_data['item_category_id'].astype(np.int8)
    tot_data['cat_code'] = tot_data['cat_code'].astype(np.int8)
    tot_data['subtype_code'] = tot_data['subtype_code'].astype(np.int8)

    print(tot_data.head())

    tot_data=lag_feature_generate(tot_data,[1,2,3],'target')
    #del(group_data)

    #mean number of items sold for every month
    group_data=tot_data.groupby(['date_block_num']).agg({'target':['mean']})
    group_data.columns=['month_avg_item_count']
    group_data.reset_index(inplace=True)
    tot_data=pd.merge(tot_data,group_data,on=['date_block_num'],how='left')
    tot_data['month_avg_item_count']=tot_data['month_avg_item_count'].astype(np.float16)
    tot_data=lag_feature_generate(tot_data,[1,2,3],'month_avg_item_count')
    ...
    group_data_mean=tot_data.groupby(item_cols).agg({'target':['mean']})
    group_data_mean.columns=['month_avg_item_id_wise_count']
    group_data_sum=tot_data.groupby(item_cols).agg({'target':['sum']})
    group_data_sum.columns=['month_sum_item_id_wise_count']
    group_data_mean.reset_index(inplace=True)
    group_data_sum.reset_index(inplace=True)
    tot_data = pd.merge(tot_data, group_data_mean, how='left', on=item_cols).fillna(0)
    tot_data = pd.merge(tot_data, group_data_sum, how='left', on=item_cols).fillna(0)
    tot_data['month_sum_item_id_wise_count']=tot_data['month_sum_item_id_wise_count'].astype(np.float16)
    tot_data['month_avg_item_id_wise_count']=tot_data['month_avg_item_id_wise_count'].astype(np.float16)
    ...
    del(group_data_mean,group_data_sum)

    #mean and sum for each shop and in each month
    shop_cols=['date_block_num','shop_id']
    group_data_mean=tot_data.groupby(shop_cols).agg({'target':['mean']})
    group_data_mean.columns=['month_avg_shop_id_wise_count']
    group_data_sum=tot_data.groupby(shop_cols).agg({'target':['sum']})
    group_data_sum.columns=['month_sum_shop_id_wise_count']
    group_data_mean.reset_index(inplace=True)
    group_data_sum.reset_index(inplace=True)
    tot_data = pd.merge(tot_data, group_data_mean, how='left', on=shop_cols).fillna(0)
    tot_data = pd.merge(tot_data, group_data_sum, how='left', on=shop_cols).fillna(0)
    tot_data['month_avg_shop_id_wise_count']=tot_data['month_avg_shop_id_wise_count'].astype(np.float16)
    tot_data['month_sum_shop_id_wise_count']=tot_data['month_sum_shop_id_wise_count'].astype(np.float16)
    ...
    group_data_mean=tot_data.groupby(item_category_cols).agg({'target':['mean']})
    group_data_mean.columns=['month_avg_item_cat_id_wise_count']
    group_data_sum=tot_data.groupby(item_category_cols).agg({'target':['sum']})
    group_data_sum.columns=['month_sum_item_cat_id_wise_count']
    group_data_mean.reset_index(inplace=True)
    group_data_sum.reset_index(inplace=True)
    tot_data = pd.merge(tot_data, group_data_mean, how='left', on=item_category_cols).fillna(0)
    tot_data = pd.merge(tot_data, group_data_sum, how='left', on=item_category_cols).fillna(0)
    tot_data['month_avg_item_cat_id_wise_count']=tot_data['month_avg_item_cat_id_wise_count'].astype(np.float16)
    tot_data['month_sum_item_cat_id_wise_count']=tot_data['month_sum_item_cat_id_wise_count'].astype(np.float16)
    tot_data=lag_feature_generate(tot_data,[1,2,3],'month_avg_item_cat_id_wise_count')
    ...
    group_data_mean=tot_data.groupby(cols_select).agg({'target':['mean']})
    group_data_mean.columns=['month_shop_item_avg_cnt']
    group_data_mean.reset_index(inplace=True)
    tot_data = pd.merge(tot_data, group_data_mean, how='left', on=cols_select).fillna(0)
    tot_data['month_shop_item_avg_cnt']=tot_data['month_shop_item_avg_cnt'].astype(np.float16)
    tot_data=lag_feature_generate(tot_data,[1,2,3],'month_shop_item_avg_cnt')
    ...
    group_data_mean=tot_data.groupby(cols_select).agg({'target':['mean']})
    group_data_mean.columns=['month_shop_cat_avg_cnt']
    group_data_sum=tot_data.groupby(cols_select).agg({'target':['sum']})
    group_data_sum.columns=['month_shop_cat_tot_cnt']
    group_data_mean.reset_index(inplace=True)
    group_data_sum.reset_index(inplace=True)
    tot_data = pd.merge(tot_data, group_data_mean, how='left', on=cols_select).fillna(0)
    tot_data = pd.merge(tot_data, group_data_sum, how='left', on=cols_select).fillna(0)
    tot_data['month_shop_cat_avg_cnt']=tot_data['month_shop_cat_avg_cnt'].astype(np.float16)
    tot_data['month_shop_cat_tot_cnt']=tot_data['month_shop_cat_tot_cnt'].astype(np.float16)
    tot_data=lag_feature_generate(tot_data,[1,2,3],'month_shop_cat_avg_cnt')
    ...
    group_data_mean=tot_data.groupby(cols_select).agg({'target':['mean']})
    group_data_sum=tot_data.groupby(cols_select).agg({'target':['sum']})
    group_data_mean.columns=['month_shop_subtype_avg_cnt']
    group_data_sum.columns=['month_shop_subtype_sum_cnt']
    group_data_mean.reset_index(inplace=True)
    group_data_sum.reset_index(inplace=True)
    tot_data = pd.merge(tot_data, group_data_mean, how='left', on=cols_select).fillna(0)
    tot_data = pd.merge(tot_data, group_data_sum, how='left', on=cols_select).fillna(0)
    tot_data['month_shop_subtype_avg_cnt']=tot_data['month_shop_subtype_avg_cnt'].astype(np.float16)
    tot_data['month_shop_subtype_sum_cnt']=tot_data['month_shop_subtype_sum_cnt'].astype(np.float16)
    tot_data=lag_feature_generate(tot_data,[1,2,3],'month_shop_subtype_avg_cnt')
    ...
    group_data_mean=tot_data.groupby(cols_select).agg({'target':['mean']})
    group_data_sum=tot_data.groupby(cols_select).agg({'target':['sum']})
    group_data_mean.columns=['month_city_avg_item_cnt']
    group_data_sum.columns=['month_city_tot_item_cnt']
    group_data_mean.reset_index(inplace=True)
    group_data_sum.reset_index(inplace=True)
    tot_data = pd.merge(tot_data, group_data_mean, how='left', on=cols_select).fillna(0)
    tot_data = pd.merge(tot_data, group_data_sum, how='left', on=cols_select).fillna(0)
    tot_data['month_city_avg_item_cnt']=tot_data['month_city_avg_item_cnt'].astype(np.float16)
    tot_data['month_city_tot_item_cnt']=tot_data['month_city_tot_item_cnt'].astype(np.float16)
    tot_data=lag_feature_generate(tot_data,[1,2,3],'month_city_avg_item_cnt')
    ...
    group_data_mean=tot_data.groupby(cols_select).agg({'target':['mean']})
    group_data_sum=tot_data.groupby(cols_select).agg({'target':['sum']})
    group_data_mean.columns=['month_item_wise_city_avg_item_cnt']
    group_data_sum.columns=['month_item_wise_city_sum_item_cnt']
    group_data_mean.reset_index(inplace=True)
    group_data_sum.reset_index(inplace=True)
    tot_data = pd.merge(tot_data, group_data_mean, how='left', on=cols_select).fillna(0)
    tot_data = pd.merge(tot_data, group_data_sum, how='left', on=cols_select).fillna(0)
    tot_data['month_item_wise_city_avg_item_cnt']=tot_data['month_item_wise_city_avg_item_cnt'].astype(np.float16)
    tot_data['month_item_wise_city_sum_item_cnt']=tot_data['month_item_wise_city_sum_item_cnt'].astype(np.float16)
    tot_data=lag_feature_generate(tot_data,[1,2,3],'month_item_wise_city_avg_item_cnt')
...
if __name__ == "__main__":
    ...
    tot_data.to_csv('../data/tot_train_test_data_corrected_v5.csv')
